...
$stmt = $pdo->prepare("
    SELECT id, title, category, short_desc, description, price, discount_price, year_made, tag
    FROM products
    WHERE id = :id
    LIMIT 1
");
...
$specStmt = $pdo->prepare("
    ...
    ORDER BY sort_order ASC, id ASC
");
$specStmt->execute([':pid' => $id]);
$specsData = $specStmt->fetchAll(PDO::FETCH_ASSOC);
...
$title         = $product['title'] ?? '';
$price         = $product['price'] ?? '';
$year_made     = $product['year_made'] ?? '';
$tag           = $product['tag'] ?? '';
$short_desc    = $product['short_desc'] ?? '';
$description   = $product['description'] ?? '';
$category      = $product['category'] ?? '';
$discount_price = $product['discount_price'] ?? '';
$has_discount   = $discount_price !== null && $discount_price !== '';

$categorySelect = in_array($category, $categories, true) ? $category : '_custom_';
$categoryCustom = in_array($category, $categories, true) ? '' : $category;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $deleteIds    = array_map('intval', $deleteImages);
    ...
    if (empty($errors)) {
        try {
            ...
            $delSpecs = $pdo->prepare("DELETE FROM product_specs WHERE product_id = :pid");
            $delSpecs->execute([':pid' => $id]);

            if (!empty($specs)) {
                $insSpecs = $pdo->prepare("
            ...
            if (!empty($deleteIds)) {
                ...
                $delSql = "DELETE FROM product_images WHERE product_id = ? AND id IN ($placeholders)";
                $delStmt = $pdo->prepare($delSql);
                $params = array_merge([$id], $deleteIds);
            ...
            if (!empty($_FILES['images']) && !empty($_FILES['images']['name'])) {
                ...
                if (!empty($images)) {
                    ...
                    $dirName  = trim(dirname($firstUrl), '/'); 

                    
                    $afterBase = preg_replace('~^assets/images/products/?~', '', $dirName);
                    if ($afterBase !== '' && $afterBase !== 'products') {
                        $subDir = $afterBase;
                    }
                }
                ...
                $uploadDirFs  = $baseFs . $subDir . '/';
                $uploadDirUrl = $baseUrl . $subDir . '/';
            ...
            if (!empty($uploadedPaths)) {
                $maxSortStmt = $pdo->prepare("
                    ...
                    WHERE product_id = :pid
                ");
                $maxSortStmt->execute([':pid' => $id]);
                $maxSort = (int)$maxSortStmt->fetchColumn();
                $sort = $maxSort + 1;

                $imgStmtIns = $pdo->prepare("
...
<head>
    ...
    <title>Edit Product - RetroByte CMS</title>
                ...
                <div class="form-grid add-product-grid">
                    ...
                    <div class="form-group images-col">
                        ...
                        <div id="main-image" class="product-image-large">
                            <?php if ($firstImageSrc): ?>
                                <img src="<?php echo htmlspecialchars($firstImageSrc); ?>" alt="">
                            <?php else: ?>
                            ...
                            <div id="thumbs" class="thumbs">
                                <?php foreach ($images as $img): ?>
                                    ...
                                    >
                                        ...
                                        <img src="<?php echo htmlspecialchars($thumbSrc); ?>" alt="">
                                    </button>
                                <?php endforeach; ?>
                            </div>
                        ...
                        <p class="help-text">
                            Upload additional images. Click the X to remove existing ones.
                        </p>
                            ...
                            <?php foreach ($specsData as $row): ?>
                                <div class="spec-row">
                                    <input
                                        ...
                                        value="<?php echo htmlspecialchars($row['spec_key'] ?? ''); ?>"
                                    >
                                    <input
                                        type="text"
                                        name="spec_value[]"
                                        ...
                                        value="<?php echo htmlspecialchars($row['spec_value'] ?? ''); ?>"
                                    >
                                    <button type="button"
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function createInputAndOpenDialog() {
        ...
        const inputId = 'new-image-' + Date.now() + '-' + Math.random().toString(36).slice(2);
        input.id = inputId;
        ...
        input.addEventListener('change', function () {
            ...
            reader.onload = function (e) {
                ...
                if (thumbsContainer.children.length === 1 && !currentMain) {
    ...
    if (thumbsContainer) {
        thumbsContainer.addEventListener('click', function (ev) {
            const thumb = ev.target.closest('.thumb');
            if (!thumb) return;

            if (ev.target.classList.contains('thumb-remove')) {
                const type = thumb.dataset.type || 'existing';
                ...
                if (type === 'existing') {
                    ...
                    const inputId = thumb.dataset.inputId;
                    if (inputId) {
